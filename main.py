...
def main(args):
    ...
    for image_full_name in image_file_name_list:
        ...
        start = time.time()
        images_and_labels_generator = augment.generate_n_augmented_images_labels(image, labels, n_transformations)
        file.write_output_files(image_name, images_and_labels_generator, output_path)
        end = time.time()
